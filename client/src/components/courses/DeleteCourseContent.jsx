...
export default function DeleteCourseContent(props) {
  const { actions, course, content, onConfirm, okBtnText, history, fetch, user } = props;
  ...
  const handleDeleteRequest = () => {
    ...
    setMessage(
      'Warning. This operation cannot be undone. Do you really want to remove the content from?',
    );
  };
  ...
  const handleClose = (confirmResult) => {
    if (mode === 'confirm' && confirmResult === true) {
      ...
      actions.courseContentRemove(course.id, content.id).then((result) => {
        setOpen(false);
        if (result.message) {
