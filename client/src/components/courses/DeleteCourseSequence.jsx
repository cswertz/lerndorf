...
export default function DeleteCourseSequence(props) {
  const { actions, course, content, onConfirm, okBtnText, history, fetch, user, sequence } = props;
  ...
  const handleDeleteRequest = () => {
    ...
    setMessage(
      'Warning. You are about to delete the micromodel. This operation cannot be undone. Are you sure?',
    );
  };
  ...
  const handleClose = (confirmResult) => {
    if (mode === 'confirm' && confirmResult === true) {
      ...
      actions.courseSequenceRemove(course.id, sequence.id).then((result) => {
        setOpen(false);
        if (result.message) {
  ...
  return (
    <>
      <IconButton aria-label="Delete sequence" onClick={handleDeleteRequest}>
        <DeleteIcon />
      </IconButton>
      <Dialog
        open={open}
